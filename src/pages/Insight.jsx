...
const ViewInsight = () => {
  ...
  const { isLoading, detailInsightData, latestInsightData } = useSelector(
    (state) => state.blog,
  );

  const data = detailInsightData?.data;
  const newsSetting = detailInsightData?.insightSetting;
  ...
  useEffect(() => {
    dispatch(getDetailInsightBySlug(slug));
  ...
  return (
    <>
      <Helmet>
        <title>{data?.insight_title || "Babvip"}</title>
        <meta
          name="description"
          content={data?.insight_short_details1 || "Babvip Description"}
        />
            ...
            <div class="container">
              <div class="row align-items-center">
                <div class="col-lg-8 col-12">
                  <h1 class="fw-bold display-5">{data?.insight_title}</h1>
                </div>
              </div>
              ...
              <div class="row justify-content-between">
                <div class="col-lg-8 pe-lg-5">
                  <div class="blog-details-wrap">
                    {parse(String(data?.insight_details1))}
                  ...
                  <blockquote class="bg-white custom-shadow p-5 mt-5 rounded-custom border-4 border-primary border-top">
                    <p class="text-muted">
                      <i class="fas fa-quote-left me-2 text-primary"></i>
                      {data?.insight_short_details2}
                      <i class="fas fa-quote-right ms-2 text-primary"></i>
                    </p>
                  </blockquote>
                  <div class="job-details-info mt-5">
                    {parse(String(data?.insight_details2))}
                  </div>
                ...
                <div class="col-lg-4">
                  <div class="author-wrap text-center bg-light-subtle p-5 sticky-sidebar rounded-custom mt-5 mt-lg-0">
                    <img
                      src={`${import.meta.env.VITE_REACT_APP_IMAGE_PATH}/${
                        newsSetting?.setting_data?.Insight_logo
                      }`}
                      alt={newsSetting?.setting_data?.Insight_logo_alt}
                      width="120"
                  ...
                  latestInsightData?.map((item, idx) => (
                    <div key={idx} class="col-lg-4 col-md-6">
                      <div class="single-article rounded-custom my-3">
                        <Link to={`/insights/${item?.insight_slug}`}>
